...
const VendorPage = () => {
  ...
  const handleAddOrUpdateVendor = async (values) => {
    try {
      if (editVendor) {
        // Update vendor
        const response = await fetch(`${process.env.REACT_APP_SERVER_URL}/api/vendors/update-vendor/${editVendor._id}`, {
          method: 'PUT',
        ...
        setVendors(vendors.map(v => (v._id === editVendor._id ? updatedVendor.vendor : v)));
        message.success("Vendor updated successfully.");
      } else {
        // Add new vendor
        const response = await fetch(`${process.env.REACT_APP_SERVER_URL}/api/vendors/add-vendor`, {
          method: 'POST',
          body: JSON.stringify(values),
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`${process.env.REACT_APP_SERVER_URL}/api/vendors/delete-vendor/${id}`, {
        method: 'DELETE',
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
    ...
    <>
      ...
      <div className="px-6 min-h-[550px]">
        ...
        <div className="flex justify-end">
          ...
          <Button type="primary" onClick={() => navigate('/vendor')} className="mb-4 ml-4">
            Go Back
          </Button>
        </div>
        ...
        >
          ...
          >
            ...
            <Form.Item
              label="Phone"
              name="phone"
              rules={[{ required: true, message: "Please input the vendor's phone number!" }]}
            >
              <Input />
            </Form.Item>
