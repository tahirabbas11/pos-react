    ...
    addProduct: (state, action) => {
      ...
      if (cartFindItem) {
        ...
        if (cartFindItem.quantity < totalQuantity) {
          ...
          const discountAmountPerPiece =
            (cartFindItem.price * cartFindItem.discount) / 100;
          cartFindItem.discountPricePerPiece =
            cartFindItem.price - discountAmountPerPiece; // Update discounted price
          cartFindItem.totalproductprice =
            cartFindItem.discountPricePerPiece * cartFindItem.quantity; // Update total product price
          message.success('Product quantity updated.');
        } else {
          message.error(
            `Cannot add more than ${totalQuantity} of this product.`
          );
        }
      } else {
        ...
        if (1 <= totalQuantity) {
          ...
          state.cartItems.push({
            ...
            discountPricePerPiece:
              action.payload.price - discountAmountPerPiece, // Discounted price per piece
      ...
      state.subtotal = state.cartItems.reduce(
        (subtotal, item) => subtotal + item.totalproductprice,
        0
      );
      ...
      if (discountType === 'product') {
        ...
        if (discount >= 0 && discount <= 100) {
          ...
          if (cartFindItem) {
            ...
            const discountAmountPerPiece =
              (cartFindItem.price * discount) / 100;
            cartFindItem.discountPricePerPiece =
              cartFindItem.price - discountAmountPerPiece; // Discounted price per piece
            cartFindItem.totalproductprice =
              cartFindItem.discountPricePerPiece * cartFindItem.quantity; // Update total product price
            ...
            message.success(
              `Discount of ${discount}% applied to ${cartFindItem.title}.`
            );
          } else {
        ...
        if (overallDiscount >= 0 && overallDiscount <= 100) {
          const overallDiscountAmount =
            (state.subtotal * overallDiscount) / 100;
      ...
      if (cartFindItem) {
        ...
        if (quantity <= totalQuantity && quantity >= 1) {
          ...
          cartFindItem.discountPricePerPiece =
            cartFindItem.price - discountAmountPerPiece; // Update discounted price
          cartFindItem.totalproductprice =
            cartFindItem.discountPricePerPiece * quantity; // Update total product price
          message.success('Product quantity updated.');
        } else {
